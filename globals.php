...
if ($method === 'GET') {
    switch ($endURI) {
        case 'allproducts':
            $result = $controller->productsController->getAllProducts();
            break;
        default:
            $result = [];
    }
    ...
    switch ($endURI) {
        case 'filterproducts':
            $result = $controller->productsController->filter($decodedData, []);
            break;
        case 'getproduct':
            $result = $controller->productsController->getProductById($decodedData);
            break;
        case 'getuser':
            $result = $controller->usersController->getUserById($decodedData);
            break;
        case 'updateuser':
            if (json_last_error() === JSON_ERROR_NONE) {
                ...
                $result = $controller->usersController->updateUser($userId, ['password' => $password]);
            }
            break;
        case 'login':
            if (json_last_error() === JSON_ERROR_NONE) {
        ...
        case 'logout':
            ...
            break;
        case 'register':
            $result = $controller->usersController->createUser($decodedData);
            break;
        case 'removeItem':
            break;
        case 'check':
            require_once "$path/transactions/checkout.php";
            $controller->addToCart('prod_PBRpRO0LQiFGuL', 2);
